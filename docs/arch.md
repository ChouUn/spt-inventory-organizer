...
- 最终排布在每个网格内按占用高度、从父到子的各层类别跨度之和加权，
  ...
  聚合模型共同处理单格与大件，类别来自游戏手册的完整祖先链。
- 排布层隔离物品身份：等价物品以匿名位置槽求解，布局确定后优先保留原位 ID，
  再把真实物品的位置变更交给编排与游戏适配层。
- 最终排布统一规划全部待整理网格，共享剩余搜索预算；各网格归属约束不变，
  规划完成后分别校验与提交，不按容器顺序各自耗用搜索额度。
- 聚合前置提示按类别、行和转置后尺寸对已有占位作数量分配，支持全网格交换；
  提示不改变固定障碍、物品归属或占用几何，后续联合模型仍可重新决定全部位置。
...
flowchart TB
    ...
    Collect --> Pack[阶段四：Packing 全部网格联合排布，逐网格校验并提交]
...
- 本地化类别链用于 tag 匹配，完整类别 ID 链用于父层优先的聚合。
